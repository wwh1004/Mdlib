...
using System.Text;
using Mdlib.PE;
using static Mdlib.DotNet.Metadata.NativeConstants;

namespace Mdlib.DotNet.Metadata {
	/// <summary>
	/// 元数据流头
	/// </summary>
	[DebuggerDisplay("StmHdr:[P:{MdlibUtils.PointerToString(RawData)} RVA:{RVA} FileOffset:{FileOffset} N:{DisplayName}]")]
	internal sealed unsafe class StreamHeader : IRawData<STORAGESTREAM> {
		...
		internal StreamHeader(IMetadata metadata, uint index) {
			...
			if (index == 0)
				_offset = (uint)metadata.StorageHeader.FileOffset + metadata.StorageHeader.Length;
			else
				_offset = (uint)metadata.StreamHeaders[index - 1].FileOffset + metadata.StreamHeaders[index - 1].Length;
