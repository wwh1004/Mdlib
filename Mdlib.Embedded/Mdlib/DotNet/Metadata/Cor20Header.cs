	...
	internal sealed unsafe class Cor20Header : IRawData<IMAGE_COR20_HEADER> {
		...
		public uint? EntryPointToken {
			get {
				...
				return RawValue->EntryPointTokenOrRVA;
			}
			set {
				if (value is null)
		...
		internal Cor20Header(IMetadata metadata) {
			...
			_offset = (uint)_peImage.ToFileOffset((RVA)_peImage.NtHeader.OptionalHeader.DataDirectories[IMAGE_DIRECTORY_ENTRY_COM_DESCRIPTOR].Address);
